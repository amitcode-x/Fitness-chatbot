...
function getFollowUps(lastUserText = "") {
  ...
  if (t.includes("weight") || t.includes("fat") || t.includes("lose"))
    return ["How many calories should I eat?", "Best cardio for fat loss?", "Can I lose weight without a gym?"];
  if (t.includes("muscle") || t.includes("gain") || t.includes("bulk"))
    return ["What's a good daily protein intake?", "Push/Pull/Legs split explained?", "Best supplements for muscle gain?"];
  if (t.includes("diet") || t.includes("eat") || t.includes("nutrition") || t.includes("meal"))
    return ["Give me a 7-day meal plan", "Best foods to eat before a workout?", "How much water should I drink daily?"];
  if (t.includes("cardio") || t.includes("run") || t.includes("endurance"))
    return ["How often should I do cardio?", "HIIT vs steady-state cardio?", "Best cardio exercises for beginners?"];
  if (t.includes("sleep") || t.includes("recover") || t.includes("rest"))
    return ["How many hours of sleep do athletes need?", "Best post-workout recovery tips?", "Should I train with sore muscles?"];
  return ["Create a weekly workout plan for me", "What should I eat after a workout?", "How do I stay consistent with fitness?"];
}

// ── Live countdown hook ──
function useCountdown(targetMs) {
  const [timeLeft, setTimeLeft] = useState(() => targetMs ? Math.max(0, targetMs - Date.now()) : 0);
  useEffect(() => {
    if (!targetMs) return;
    const iv = setInterval(() => setTimeLeft(Math.max(0, targetMs - Date.now())), 1000);
    return () => clearInterval(iv);
  }, [targetMs]);

  if (!timeLeft || timeLeft <= 0) return "Starting now! 🎉";
...
function LimitCard({ data }) {
  ...
  const isMinute = data.type === "minute";
  const countdown = useCountdown(isDaily ? data.resetAt : Date.now() + (data.msLeft || 60000));

  return (
    <>
      <style>{`
        @keyframes limitIn { from{opacity:0;transform:translateY(16px) scale(0.97)} to{opacity:1;transform:translateY(0) scale(1)} }
        .limit-card-anim { animation: limitIn 0.4s cubic-bezier(0.22,1,0.36,1) both }
        @keyframes cPulse { 0%,100%{opacity:1} 50%{opacity:0.55} }
        .c-pulse { animation: cPulse 1s ease-in-out infinite }
        @keyframes pGlow { 0%,100%{box-shadow:0 0 8px rgba(239,68,68,0.3)} 50%{box-shadow:0 0 16px rgba(239,68,68,0.6)} }
        ...
        <div className="max-w-[80%] md:max-w-[65%] rounded-2xl rounded-bl-sm border overflow-hidden"
          ...
          <div className="px-4 py-2.5 flex items-center gap-2"
            ...
            <span className="text-sm font-bold" style={{ fontFamily: "'Syne',sans-serif", color: "#fca5a5" }}>
              {isDaily ? "Daily Limit Reached" : isMinute ? "Slow Down!" : "Rate Limited"}
            </span>
          </div>
          ...
          <div className="px-4 py-3">
            ...
            <p className="text-sm leading-relaxed mb-3" style={{ color: "rgba(255,255,255,0.75)", fontWeight: 300 }}>
              {isDaily
                ? "You've used all 250 messages for today. Your session will reset at midnight."
                : isMinute
                ? "You're sending messages too fast. Please wait a moment before continuing."
                : "The server is temporarily rate-limited. Please wait a moment and try again."}
            </p>
            ...
            <div className="flex items-center gap-3 mb-3 p-2.5 rounded-xl"
              ...
              <div>
                <div className="text-xs mb-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
                  {isDaily ? "New session starts in" : "You can chat again in"}
                </div>
                <div className="c-pulse text-sm font-bold" style={{ fontFamily: "'Syne',sans-serif", color: "#fca5a5" }}>
                  {countdown}
                </div>
              </div>
              {isDaily && data.resetTimeStr && (
                <div className="ml-auto text-right">
                  <div className="text-xs mb-0.5" style={{ color: "rgba(255,255,255,0.25)" }}>Resets at</div>
                  <div className="text-xs font-bold" style={{ fontFamily: "'Syne',sans-serif", color: "rgba(252,165,165,0.7)" }}>
                    {data.resetTimeStr}
                  </div>
                </div>
              )}
            ...
            {isDaily && (
              <div className="mb-3">
                ...
                <div className="rounded-full overflow-hidden" style={{ height: 5, background: "rgba(255,255,255,0.08)" }}>
                  <div className="p-glow h-full rounded-full"
            ...
            {/* Tip */}
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.25)", fontStyle: "italic" }}>
              {isDaily
                ? "💡 While you wait, review your workout plan or track your nutrition!"
                : "💡 Take a breath — rest is an important part of training too! 😄"}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="max-w-3xl mx-auto px-4 py-3.5 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <div className="border rounded-full px-3 py-1.5 flex items-center gap-2"
                ...
                <span className="hidden sm:inline text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>💬 {userCount}</span>
                <div className="w-px h-3 hidden sm:block" style={{ background: "rgba(255,255,255,0.08)" }} />
          ...
          <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
            ...
            {messages.length === 0 && (
              <div className="fade-up flex flex-col items-center text-center pt-8 pb-4">
                ...
                {/* Usage hint */}
                <div className="mt-6 px-4 py-2 rounded-xl"
                  style={{ background: "rgba(52,211,153,0.04)", border: "1px solid rgba(52,211,153,0.1)" }}>
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)" }}>
                    📊 {usageStats.dailyRemaining} messages remaining today
                  </span>
                </div>
              </div>
            )}
            ...
            {messages.map((msg, i) => {
              if (msg.role === "limit") return <LimitCard key={i} data={msg.limitData} />;
